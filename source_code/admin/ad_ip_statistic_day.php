      <?php
                if(isset($_POST["sub_day_import"]))
				{
			
					$day_star_im=$_POST['import_start_day'];
					$day_end_im=$_POST['import_end_day'];
					
					if(strtotime($day_star_im)>strtotime($day_end_im))
					{
						$temp=$day_star_im;
						$day_star_im=$day_end_im;
						$day_end_im=$temp;
				...
				{
					$day_star_im=$_GET['day_star_im'];
					$day_end_im=$_GET['day_end_im'];
				}
				$list_order=$ad_select->query_statistic_day_import($day_star_im,$day_end_im);
				$count=count($list_order);
				$limit=4;
				$posision=$ad_pager->findStart($limit);
				$pag=$ad_pager->findPages($count,$limit);
				$param="&day_star_im=$day_star_im&day_end_im=$day_end_im";
				$current=$_GET["page"];
			
				$pagination=$ad_pager->pageList($current,$pag,$param);
	
				$list_all_import_follow_day=$ad_select->query_statistic_day_import_page($day_star_im,$day_end_im,$posision,$limit);
				$total_money=0;
				$total_finish=0;
				$total_product=0;
				...
				?>
                   <div class="col-xs-12  col-lg-8 col-lg-offset-2">
                    <div class="panel panel-default">
                          <div class="panel-heading">  <h5 class="text-center">DANH SÁCH PHIẾU NHẬP TRONG KHOẢNG THỜI GIAN 
                        	<?php echo date("d-m-Y",strtotime($day_star_im))." ĐẾN ".date("d-m-Y",strtotime($day_end_im));?></h5></div>
                        ...
                        <div class="table-responsive">
                              <table class="table table-hover table-striped">
                                ...
                                <tbody>
                                <?php
                                	foreach($list_all_import_follow_day as $lst_im_bill)
									{
								?>
                                    <tr>
