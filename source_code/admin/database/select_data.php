<?php
	...
	class select_data extends database
	{
		...
		{
			$pdo=parent::conect_data();
			$query="SELECT SUM(detail_info_order.quantity_one_product) as Tong_so,product_info.image_product,product_info.name_product,product_info.price_product,product_info.point_promotion,product_info.id_product FROM `detail_info_order` INNER JOIN product_info ON product_info.id_product=detail_info_order.product WHERE product_info.catalog_product = $catalog_product AND product_info.quantity_inventory > 0  AND product_info.status_product=0 GROUP BY product ORDER BY Tong_so DESC LIMIT 0,6";
			$PDO=$pdo->prepare($query);
			$PDO->execute();
			$info_product=$PDO->fetchAll(PDO::FETCH_OBJ);
			return $info_product;
		...
		{
			$pdo=parent::conect_data();
			$query="SELECT * FROM product_info WHERE quantity_inventory > 0  AND product_info.status_product=0 ORDER BY product_info.point_promotion DESC LIMIT 0,6";
			$PDO=$pdo->prepare($query);
			$PDO->execute();
			$info_product=$PDO->fetchAll(PDO::FETCH_OBJ);
			return $info_product;
