      <?php
                if(isset($_POST["sub_day_order"]))
				{
			
					$day_star_order=$_POST['start_day_order'];
					$day_end_order=$_POST['end_day_order'];
					
					if(strtotime($day_star_order)>strtotime($day_end_order))
					{
						$temp=$day_star_order;
						$day_star_order=$day_end_order;
						$day_end_order=$temp;
				...
				{
					$day_star_order=$_GET['start_day_or'];
					$day_end_order=$_GET['day_end_order'];
				}
				$list_order=$ad_select->query_statistic_day_order($day_star_order,$day_end_order);
				$count=count($list_order);
				$limit=10;
				$posision=$ad_pager->findStart($limit);
				$pag=$ad_pager->findPages($count,$limit);
				$param="&start_day_or=$day_star_order&day_end_order=$day_end_order";
				$current=$_GET["page"];
			
				$pagination=$ad_pager->pageList($current,$pag,$param);
	
				$list_all_order_follow_day=$ad_select->query_statistic_day_order_page($day_star_order,$day_end_order,$posision,$limit);
				$total_money=0;
				...
				?>
                   <div class="col-xs-12  col-lg-8 col-lg-offset-2">
                    <div class="panel panel-default">
                          <div class="panel-heading">  <h5 class="text-center">DANH SÁCH ĐƠN HÀNG TRONG KHOẢNG THỜI GIAN 
                        	<?php echo date("d-m-Y",strtotime($day_star_order))." ĐẾN ".date("d-m-Y",strtotime($day_end_order));?></h5></div>
